...
namespace Unity.Services.RemoteConfig.Tests.Editor.Authoring.Analytics
{
    class ConfigAnalyticsTests
    {
        ...
        {
            var configAnalytics = new ConfigAnalytics(m_MockAnalyticsWrapper.Object);
            ...
            int expectedGroupCount)
        {
            ...
            var validFiles = new List<RemoteConfigFile>();
            for (var i = 0; i < nbSuccessfulFiles; i++)
            {
                validFiles.Add(TestFileGetter.GetTestFile(AssetFilePaths.ValidBase));
            }
