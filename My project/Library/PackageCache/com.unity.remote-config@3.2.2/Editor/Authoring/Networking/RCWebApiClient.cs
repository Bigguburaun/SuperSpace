...
namespace Unity.Services.RemoteConfig.Authoring.Editor.Networking
{
    ...
    class RcWebApiClient : IWebApiClient, IDisposable
    {
        ...
        public async Task<FetchResult> Fetch(string cloudProjectId, string environmentId)
        {
            ...
            return new FetchResult(result as JObject);;
        }

        public async Task<string> Post(string cloudProjectId, string environmentId, JArray configs)
        {
            var request = new RcWebApiClientRequest();
            m_ActiveRequests.Add(request);
            var result = await request.Post(cloudProjectId, environmentId, configs);
            m_ActiveRequests.Remove(request);
            return result as string;
        }

        public async Task Put(string cloudProjectId, string environmentId, string configId, JArray configs)
        {
            var request = new RcWebApiClientRequest();
            m_ActiveRequests.Add(request);
            var result = await request.Put(cloudProjectId, environmentId, configId, configs);
            m_ActiveRequests.Remove(request);
        }
